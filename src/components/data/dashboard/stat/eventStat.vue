  ...
  <div class="px-[25px] py-[10px] rounded-[26px] shadow-sm bg-white">
    <div class="flex justify-between items-center">
      ...
      >
        Мероприятия
        <span
          class="absolute bg-[#FAF4FE] py-[2px] px-[15px] rounded-2xl text-[15px] text-[#B6A3D0] top-[-10px]"
          >{{ e?.count || 0 }}</span
        >
      </div>
      <div class="w-[45px]">
    ...
    >
      <div
        ...
        :key="key"
        class="col-span-4 flex justify-between items-center py-2 border-gray-200 cursor-pointer lg:col-span-6 xm:col-span-12"
        :class="{ 'border-b': index < Object.entries(filteredItems).length - 3 }"
        @click="$router.push({ name: 'event', query: { queryType: key } })"
      >
        <!-- {{ key }} -->
        <div
          class="flex gap-2 text-[#483D5B] text-[18px] items-center font-semibold text-sm xs:text-[11px]"
        >
          <div v-if="data[key]?.icon" class="bg-[#F3EBFC] rounded-lg p-[7px]">
            <component
              v-if="data[key]?.icon"
              :is="data[key]?.icon"
              class="size-6 text-[#9E55EC]"
              aria-hidden="true"
            />
          </div>
          {{ data[key]?.title }}
