<template>
  <div class=" text-sm flex flex-col overflow-hidden">
    <h3 class="text-[16px] font-bold leading-6 text-[#483D5B] w-full mb-4">Статистика по новостям</h3>
...
const getWorkerNewsStat = async () => {
  ...
  const { data } = await store.getWorkerNewsStat({
    _id: id.value
  })
