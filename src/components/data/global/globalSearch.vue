          ...
          >
            <Combobox @update:modelValue="onSelect">
              <div class="relative">
                ...
                />
                <ComboboxInput
                  class="h-12 w-full border-0 bg-transparent pl-11 pr-4 text-gray-900 placeholder:text-gray-400 sm:text-sm focus:ring-0 focus:border-0 focus-visible:border-0"
                  placeholder="Search..."
...
const searching = async () => {
  if (searchText.value?.length < 4) return false
  console.log(searchText.value)
  const result = await logger_store.globalSearch(searchText.value)
  console.log(result)
}
